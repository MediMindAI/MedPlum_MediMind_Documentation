...
### Content Loading (Page-Per-Chapter)

The site uses a **category-scoped** loading model — each chapter (category) gets exclusive content view. Navigating between chapters replaces the content rather than accumulating all sections.

`SectionLoader` (`js/section-loader.js`):
1. Reads category/section structure from `config/manifest.json`
2. `loadCategory(categoryId)` clears `#sectionsContainer`, inserts a translated H1 chapter title, then fetches only that category's unique files in parallel
3. Route changes (`_handleRouteChange`) swap categories when needed, or scroll to anchors within the same category
4. Language switching reloads the active category only
5. Uses LRU cache with TTL for performance
6. Sub-sections loaded into containers (e.g., `architecture-technical`)

`TocGenerator` (`js/toc-generator.js`):
- Category clicks trigger `Router.navigate()` to load that chapter
- Non-active categories are collapsed automatically
...
| `router.js` | Hash-based routing, scrolls to top on category-only routes |
...
| `toc-generator.js` | Generates TOC from manifest, category clicks navigate chapters |
| `section-loader.js` | Category-scoped section loading (page-per-chapter) |
